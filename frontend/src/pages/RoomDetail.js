...
const RoomDetail = () => {
    ...
    const loadRoomData = useCallback(async () => {
        try {
            ...
            const [roomRes, sensorsRes, sensorsTypes] = await Promise.all([
                axios.get(`http://localhost:5000/rooms/${room_id}`, { data: { token } }),
                axios.get(`http://localhost:5000/sensors?room_id=${room_id}`, { data: { token } }),
                axios.get(`http://localhost:5000/sensor-types`, { data: { token } }),
            ]);

            setRoom(roomRes.data);
            setSensors(sensorsRes.data);
            setSensorTypes(sensorsTypes.data);
            setOriginalSensors(sensorsRes.data);
            console.log(sensorsRes.data);
            ...
            try {
                ...
                if (error.response && error.response.status === 404) {
                    console.warn("Карта не найдена, вы можете добавить новую карту.");
                    setMapImage(null);
                } else {
    ...
    useEffect(() => {
        const hasAlert = sensors.some(sensor => sensor.is_alert);
        setShowAlertModal(hasAlert);
    }, [sensors]);
    ...
    useEffect(() => {
        ...
        if (mapImage) {
            ...
            sensors.forEach(sensor => {
                ...
                if (sensor.is_alert) {
                    ctx.fillStyle = selectedSensor?.sens_id === sensor.sens_id ? 'rgba(255, 0, 0, 1)' : (blink ? 'rgba(255, 0, 0, 0.5)' : 'rgba(255, 0, 0, 0.1)');
                } else {
                    ctx.fillStyle = selectedSensor?.sens_id === sensor.sens_id ? 'rgba(0, 0, 255, 0.5)' : 'rgba(0, 255, 0, 0.5)';
                }
    ...
    }, [mapImage, sensors, selectedSensor, scale, blink]);
    ...
    const saveChanges = async () => {
        ...
        try {
            ...
            const updatedSensors = sensors.filter(sensor => {
                const original = originalSensors.find(s => s.sens_id === sensor.sens_id);
                if (!original) {
                    console.warn(`Сенсор с ID ${sensor.sens_id} не найден в originalSensors`);
                    return false;
                }
    ...
    const handleRandomRequest = async () => {
        try {
            const response = await axios.post('http://localhost:5000/rand', {
                room_id: room_id
            }, {
                headers: {
    ...
    const handleNoAlert = async () => {
        try {
            const response = await axios.post('http://localhost:5000/noAlert', {
                room_id: room_id 
            }, {
                headers: {
                    'Content-Type': 'application/json'
    ...
    return (
        <div className="room-detail">
            ...
            <div className="sensor-controls">
                ...
                <button onClick={handleRandomRequest}>ТЕСТ (СЛУЧАЙНЫЙ ДАТЧИК)</button>
                {selectedSensor && (
                    <>
                        ...
                        <div className="sensor-info">
                            ...
                            <p>Тревога: {selectedSensor.is_alert === "t" ? "Да" : "Нет"}</p>
                        </div>
                    </>
                )}
                {hasChanges && (
            ...
            {showAlertModal && (
                <div className="alert-modal">
                    <div className="alert-modal-content">
                        <h3>Внимание!</h3>
                        <p>Один или несколько датчиков бьют тревогу.</p>
                        <button onClick={() => setShowAlertModal(false)}>Закрыть</button>
                    </div>
                </div>
            )}
            ...
            <button onClick={handleNoAlert} className='no-alert'>
                Отключить тревогу
            </button>
        </div>
    );
